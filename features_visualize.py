...
if __name__ == "__main__":
    ...
    for img_path in os.listdir(dir_path):
        ...
        print(img_path)
        image = cv2.imread(r"E:\pycharm\Project\Bamboo\MosoBamboo\AgeDetect\Data\OriDataset\1\IMG_3009.JPG")
        ...
        # 注册钩子
        model.color_stream.blocks.register_forward_hook(get_features('color_blocks'))
        model.texture_stream.fpn.register_forward_hook(get_features('texture_fpn'))
        model.texture_stream.cbam.register_forward_hook(get_features('texture_cbam'))

        # 运行模型
        model.eval()
        output = model(new_img, gray_image)


        # 定义可视化函数
        def visualize_feature_map(feature_map, title):
            feature_map = feature_map[0]  # 选择第一个样本
            feature_map_mean = torch.mean(feature_map, dim=0).detach().cpu().numpy()
        ...
        # 可视化特征图
        visualize_feature_map(features['color_blocks'], 'Color Stream Blocks Feature Map')
        visualize_feature_map(features['texture_fpn'], 'Texture Stream FPN Feature Map')
        visualize_feature_map(features['texture_cbam'], 'Texture Stream CBAM Feature Map')
